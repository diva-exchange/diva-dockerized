...
then
  ...
  mkdir -p "${PATH_DOMAIN}"/db/chain
  mkdir -p "${PATH_DOMAIN}"/db/state
fi
cd "${PATH_DOMAIN}"
...
then
  ...
  sudo rm -rf "${PATH_DOMAIN}"/db/state/*
  sudo rm -rf "${PATH_DOMAIN}"/db/chain/*
fi

if [[ ! -f genesis/local.config ]]
then
  running "Creating Genesis TX using I2P"
  ...
  # handle joining, replace genesis tx with a dummy
  if [[ -n ${JOIN_NETWORK} ]]
  then
    rm -rf ./genesis/tx.v1.json
    cp "${PROJECT_PATH}"/build/dummy.tx.v1.json ./genesis/tx.v1.json
  fi

  ok "Genesis TX successfully created"
fi
